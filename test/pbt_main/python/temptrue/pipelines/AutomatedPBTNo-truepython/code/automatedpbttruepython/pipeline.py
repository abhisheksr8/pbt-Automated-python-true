...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/AutomatedPBTNo-truepython")
    registerUDFs(spark)

    try:
        
        MetricsCollector.start(spark = spark, pipelineId = "pipelines/AutomatedPBTNo-truepython", config = Config)
    except :
        
        MetricsCollector.start(spark = spark, pipelineId = "pipelines/AutomatedPBTNo-truepython")
